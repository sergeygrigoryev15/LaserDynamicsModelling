...
public class KernelBuilder {
    ...
    private static String mainKernelPartTemplate = "__kernel void %1%s(__global float* inPLUS, __global float* outPLUS,__global float* inMINUS,__global float* outMINUS){\n%2%s}";
    ...
    private String defineFunctions(){
        ...
        String str =
                ...
                "float d_AM_func(float d_AM, float uPLUS, float uMINUS, float x){treturn ((1/AM_lifeTime)+(absorption*((pump_power*pump_waveLength)/(pump_crossSection*h*c))*exp((-1)*absorption*AM_ionsConc*x)))*(1-(1+(exp(uPLUS) + exp(uMINUS)))*d_AM);}\n";
        builder.append(str);
        return builder.toString();
    }
    ...
    private String defineMainPart(){
        ...
        return String.format(mainKernelPartTemplate, Constants.kernelName, str);
    }
}
